...
namespace MageSpark\Base\Plugin\Frontend;

use MageSpark\Base\Model\LessToCss\Config\Converter;
...
use MageSpark\Base\Model\LessToCss\Config as LessToCssConfig;
 ...
 * Class AddAssets
 *
 * @package MageSpark\Base\Plugin\Frontend
 */
class AddAssets
{
    const CACHE_KEY = 'magespark_should_load_css_files';
    /**
     *
     * @var array
     */
     ...
     * @param LessToCssConfig $lessConfig
        ...
        if ($shouldLoad) {
            ...
            foreach ($modulesConfig as $moduleName => $moduleConfig) {
                foreach ($moduleConfig[Converter::IFCONFIG] as $configPath) {
                    if (!$this->isSetFlag($configPath, ScopeInterface::SCOPE_STORE)) {
                        continue 2;
                    }
                }
    ...
    {
        // i.e. 'MageSpark_Checkout::css/styles.css'
