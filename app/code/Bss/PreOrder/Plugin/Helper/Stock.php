...
use Bss\PreOrder\Model\MSI\MsiObjectManagement;
...
class Stock
{
     ...
     * @param MsiObjectManagement $msiObjectManagement
     */
    public function __construct(
        PreOrderHelper $preOrderHelper,
        \Magento\Store\Model\StoreManagerInterface $storeManager,
        MsiObjectManagement $msiObjectManagement
    ) {
    ...
    public function afterAddIsInStockFilterToCollection(
        ...
        if ($this->preOrderHelper->isEnable()
            ...
            if ($this->msiObjectManagement->isMsiEnabled()) {
                $stockId = $this->msiObjectManagement->getStockIdForWebsite()->execute();
                $defaultStockId = $this->msiObjectManagement->getDefaultStock()->getId();
            } else {
                $stockId = 1;
                $defaultStockId = 1;
            }
