...
use Digidirect\Navigation\Ui\DataProvider\Menu\Form\Modifier\System;
...
class SystemTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testModifyData($data, $expected)
    {
        ...
        $this->_urlBuilderMock->expects($this->at(0))
            ->method('getUrl')
            ->with('digidirect_navigation/menu/save', ['store' => 1, 'id' => 11])
            ->willReturn('some/save/url/store/1/id/11');

        $this->_urlBuilderMock->expects($this->at(1))
            ->method('getUrl')
            ->with('digidirect_navigation/menu/validate', ['store' => 1, 'id' => 11])
