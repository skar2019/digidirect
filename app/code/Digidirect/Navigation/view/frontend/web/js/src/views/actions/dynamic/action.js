...
export default class Action {
    ...
    constructor(options, viewCore) {
        ...
        this.viewModel = function (options) {
            ...
            this.displayMode = (item) => this._isLink(item) ? 'Digidirect_Navigation/common/link' : 'Digidirect_Navigation/common/span';
        };
    ...
    _renderTemplate() {
        var self = this;
        let template,
            path = 'text!Digidirect_Navigation/template/view/action.html';
        try {
            require([path], (action) => {
                template = action;
