...
namespace Digidirect\Feed\Model\Rule\Condition;
...
class Product extends \Magento\Rule\Model\Condition\AbstractCondition
{
    ...
    protected function _prepareValueOptions()
    {
        ...
        if ($this->getAttribute() === 'attribute_set_id') {
            ...
            $selectOptions = $this->sourceYesNo->create()->toOptionArray();
    ...
    public function collectValidatedAttributes($productCollection)
    {
        ...
        if (!in_array($attribute,
            ['category_ids', 'qty', 'php', 'is_in_stock', 'manage_stock', 'status_parent', 'is_salable'])
        ) {
    ...
    public function validate(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        switch ($attrCode) {
            ...
            case 'status_parent':
                //if at least one parent has status enabled - enabled
                //if has parents and everyone is disabled - disabled
                //if has no parent - self status
                /**
                ...
                $collection->getSelect()
                    ->join(
                        ...
                        implode(' AND ', [
                            $connection->quoteInto('relation.child_id = ?', $object->getId()),
                            'e.row_id = relation.parent_id'
                        ]),
                        []
                    );
                $collection->addAttributeToSelect('status');
                if (count($collection)) {
                    $value = SourceStatus::STATUS_DISABLED;
                    foreach ($collection as $product) {
                        if ($product->getStatus() == SourceStatus::STATUS_ENABLED) {
                            $value = SourceStatus::STATUS_ENABLED;
                            break;
                        }
                    }
                } else {
            ...
            case 'qty':
                if ($object->getTypeId() == 'configurable') {
                    ...
                    $stockItem->getResource()->load($stockItem, $object->getId(), 'product_id');
            ...
            default:
                if (!isset($this->entityAttributeValues[$object->getId()])) {
                    ...
                    foreach ($this->entityAttributeValues[$object->getId()] as $storeId => $value) {
                        $attr = $object->getResource()->getAttribute($attrCode);
