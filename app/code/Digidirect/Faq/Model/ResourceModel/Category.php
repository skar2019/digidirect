...
use Digidirect\Faq\Model\Category as FaqCategory;
...
class Category extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('digidirect_faq_category', 'entity_id');
    }
    ...
    public function lookupStoreIds($categoryId)
    {
        ...
        $select = $connection->select()
            ->from(['cps' => $this->getTable('digidirect_faq_category_store')], 'store_id')
            ->join(
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['digidirect_faq_category_store' => $this->getTable('digidirect_faq_category_store')],
                $this->getMainTable() . '.' . $linkField . ' = digidirect_faq_category_store.' . $linkField,
                []
            )
                ->where('is_active = ?', 1)
                ->where('digidirect_faq_category_store.store_id IN (?)', $storeIds)
                ->order('digidirect_faq_category_store.store_id DESC')
                ->limit(1);
        }
    ...
    public function lookupQuestionIds($categoryId)
    {
        ...
        $linkField = $entityMetadata->getLinkField();
        $select = $connection->select()
            ->from(['fcr' => $this->getTable('digidirect_faq_category_relation')], 'faq_id')
            ->join(
                ['c' => $this->getMainTable()],
