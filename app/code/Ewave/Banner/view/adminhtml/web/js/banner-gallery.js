...
define([
    ...
    $.widget('mage.bannerGallery', {
        options: {
            ...
            types: null,
            initialized: false,
            fieldImageRole: '.field-image-role'
        },
        ...
        _addItem: function (event, imageData) {
            ...
            imageData = $.extend({
                ...
                'position': count + 1,
                'sizeLabel': imageData.size ? bytesToSize(imageData.size) : ''
            }, imageData);
            ...
            imgElement.on('load', this._updateImageDimesions.bind(this, element, imageData));
        ...
        _updateImagesRoles: function () {
            ...
            $images.forEach(function (img) {
                ...
                $img.find('[data-role=roles-labels] li:not(.item-role-no_role)').each(function (index, elem) {
                    var $elem = $(elem),
                        roleCode = $elem.data('roleCode'),
                        role = roles[roleCode];

                    if (data.video_roles) {
                        if (data.video_roles.length && data.video_roles.some(function (r) {
                            return r === roleCode;
                        })) {
                            $elem.show();
                            noRole = true;
                ...
                if (noRole) {
                    $img.find('[data-role=roles-labels] [data-role-code="no_role"]').hide();
                } else {
                    $img.find('[data-role=roles-labels] [data-role-code="no_role"]').show();
                }
            });
        },
         ...
         * @param {jQeuryCollection} imageData
         */
        _updateImageDimesions: function (imgContainer, imageData) {
            var $img = imgContainer.find(this.options.imageElementSelector)[0],
                $dimens = imgContainer.find('[data-role=image-dimens]');

            if (imageData['media_type'] === 'external-video') {
                if (imageData.video_width || imageData.video_height) {
                    $dimens.text(imageData.video_width + 'x' + imageData.video_height + ' px');
                } else {
                    $dimens.text(imgContainer.find('.video_width').val() + 'x' + imgContainer.find('.video_height').val() + ' px');
                }
            } else {
        ...
        _updateImageTitle: function (event, data) {
            ...
            value = imageData['media_type'] === 'external-video' ? imageData['video_title'] : imageData.label;
    ...
    // Extension for mage.bannerGallery - Add advanced settings block
    $.widget('mage.bannerGallery', $.mage.bannerGallery, {
        ...
        _initDialog: function () {
            var self = this,
                $dialog = $(this.dialogContainerTmpl());
            ...
            $dialog.on('change', '[data-role=type-selector]', function (e, stop) {
                var $imageContainer = $dialog.data('imageContainer'),
                    parent = $(this).closest('.item'),
                    selectedClass = 'selected',
                    $checked = parent.siblings().find('[data-role=type-selector]:checked');

                if (stop === undefined) {
                    if ($checked.length) {
                        $checked.prop('checked', false).trigger('change', [true]);
                    }
                    $imageContainer.find('.roles').val($(this).val());

                    parent.toggleClass(selectedClass, $(this).prop('checked')).siblings().toggleClass(selectedClass, false);

                    if ($(this).prop('checked')) {
                        $dialog.data('imageData').widget_type = $(this).closest(self.options.fieldImageRole).data('imageRole');
                        $imageContainer.find('.widget-type').val($(this).closest(self.options.fieldImageRole).data('imageRole'));
                    } else {
                        $dialog.data('imageData').widget_type = '';
                        $imageContainer.find('.widget-type').val('');
                    }
                    self.toggleRoleType($(this), $(this).closest(self.options.fieldImageRole).data('imageRole'));
                }
            });
        ...
        onDialogOpen: function (event) {
            ...
            $(event.target).find('[data-role=type-selector]').each($.proxy(function (index, checkbox) {
            ...
            this.toggleRoleType($(event.target).find('[data-role=type-selector]'), imageData.widget_type);
            $(event.target).find('.loading-mask').delay(400).queue(function () {
                $(this).remove();
            });
        },
        ...
        toggleRoleType: function (item, role) {
            var image = _.find(this.element.find(this.options.imageSelector), function (img) {
                return $(img).not('.video-item').find('.widget-type').val() !== '';
            });

            if (role) {
                if (item.prop('checked')) {
                    item.closest('[data-image-role="' + role + '"]').siblings(this.options.fieldImageRole).hide();
                } else {
                    item.closest('[data-image-role="' + role + '"]').siblings(this.options.fieldImageRole).show();
                }
            }
            if (image) {
                item.closest('[data-image-role="' + $(image).find('.widget-type').val() + '"]').siblings(this.options.fieldImageRole).hide();
            }
        }
    });
