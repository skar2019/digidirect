...
use Ewave\StoreLocator\Helper\AeIndex;
...
class EntitiesSorter
{
    ...
    const AE_MAIN_TABLE = 'ewave_abstractentity_entity';
    ...
    public function sort(array $entities)
    {
        ...
        foreach ($entities as $entityName => $entityData) {
            if (!empty($entityData['items'])) {
                foreach ($entityData['items'] as $entity) {
        ...
        foreach ($items as $item) {
            if (!isset($entities[$item[self::ENTITY_TYPE_KEY]])) {
                $entities[$item[self::ENTITY_TYPE_KEY]] = ['items' => [], 'settings' => []];
            }
            $entities[$item[self::ENTITY_TYPE_KEY]]['items'][] = $item;
        }
        return $entities;
    }
