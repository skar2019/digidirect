<?php
namespace Ewave\AbstractEntity\Plugin\Framework\Acl\AclResource\Config\Reader;

use Ewave\AbstractEntity\Controller\Adminhtml\AbstractEntity as AbstractEntityController;
use Ewave\AbstractEntity\Controller\Adminhtml\AbstractEntity\Save as SaveController;
use Ewave\AbstractEntity\Controller\Adminhtml\AbstractEntity\Delete as DeleteController;
        ...
        foreach ($result['config']['acl']['resources'] as &$adminResource) {
            if ($adminResource['id'] == 'Magento_Backend::admin') {
                foreach ($adminResource['children'] as &$adminAcl) {
                    if ($adminAcl['id'] == 'Ewave_Utilities::ewave') {
                        foreach ($adminAcl['children'] as &$ewaveAcl) {
                            if ($ewaveAcl['id'] == 'Ewave_AbstractEntity::menu') {
                                foreach ($sets as $set) {
