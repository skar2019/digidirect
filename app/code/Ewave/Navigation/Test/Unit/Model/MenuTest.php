<?php
namespace Ewave\Navigation\Test\Unit\Model;
...
class MenuTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testGetUrl($data, $urlParameters)
    {
        ...
        $helperMock = $this->getMockObjectWithoutConstructor(
            'Ewave\Navigation\Helper\Data',
            ['getMenuItemUrl']
        );

        $categoryTypeInstance = $this->_objectManager->getObject(
            'Ewave\Navigation\Model\Menu\Type\Category',
            ['helper' => $helperMock]
        );

        $cmsBlockTypeInstance = $this->_objectManager->getObject(
            'Ewave\Navigation\Model\Menu\Type\CmsBlock',
            ['helper' => $helperMock]
        );

        $customLinkTypeInstance = $this->_objectManager->getObject(
            'Ewave\Navigation\Model\Menu\Type\CustomLinkAbstract',
            ['helper' => $helperMock]
        );
        ...
        $poolMock = $this->getMockObjectWithoutConstructor('Ewave\Navigation\Model\Menu\Type\Pool', ['get']);
        ...
        $menuModel = $this->_objectManager->getObject(
            ...
            [
                'data' => [
                    ...
                    'type_code' => $type,
                    'type_instance' => [
                        'category' => 'Ewave\Navigation\Model\Menu\Type\Category',
                        'cms_block' => 'Ewave\Navigation\Model\Menu\Type\CmsBlock',
                        'custom_link' => 'Ewave\Navigation\Model\Menu\Type\CustomLinkAbstract'
                    ]
                ],
                'helper' => $helperMock,
                'pool' => $poolMock
    ...
    public function testValidate($link, $expected)
    {
        ...
        $menuModelReflection = $this->_createReflectionClass('Ewave\Navigation\Model\Menu');
        ...
        $dataProperty->setValue(
            $menuModel,
            ['type_instance' => ['custom_link' => 'Ewave\Navigation\Model\Menu\Type\CustomLinkAbstract']]
        );

        $poolMock = $this->getMockObjectWithoutConstructor(
            'Ewave\Navigation\Model\Menu\Type\Pool',
            ['get']
        );
