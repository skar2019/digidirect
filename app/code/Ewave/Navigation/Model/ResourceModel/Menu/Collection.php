...
use Ewave\Navigation\Model\Config\Source\Type;
...
{
    const MENU_ITEM_INFO_TABLE = 'ewave_navigation_menu_item_info';
    ...
    {
        $this->_init('Ewave\Navigation\Model\Menu', 'Ewave\Navigation\Model\ResourceModel\Menu');
    }
    ...
    protected function _afterLoad()
    {
        ...
        if (!empty($allIds)) {
            ...
            foreach ($this as $item) {
                /**
                 * @var $item \Ewave\Navigation\Model\Menu
                 */
    ...
    {
        $this->getSelect()->joinInner(
            ['set_link_table' => $this->getTable('ewave_navigation_menu_set_link')],
        ...
        $this->getSelect()->joinInner(
            ['set_table' => $this->getTable('ewave_navigation_menu_set')],
            'set_link_table.menu_set_id = set_table.set_id AND set_table.status = 1 AND ' . $codeCondition,
            []
        );

        $this->getSelect()->joinInner(
            ['set_store_table' => $this->getTable('ewave_navigation_menu_set_store')],
    ...
    public function addStoreFilter($storeId = null)
    {
        ...
        $this->getSelect()->joinLeft(
            ['mst' => 'ewave_navigation_menu_item_store_relation'],
