<?php
namespace Ewave\LayeredNavigation\Test\Unit\Controller;
...
class RouterTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * Router mock
     * @var \Ewave\LayeredNavigation\Controller\Router
     */
    protected $_router;
    ...
    /**
     * @var \Ewave\LayeredNavigation\Helper\Url
     */
    protected $_urlHelper;

    /**
     * @var \Ewave\LayeredNavigation\Helper\UrlParser
     */
    ...
    public function setUp()
    {
        ...
        $this->_urlHelper = $this->getMockBuilder('\Ewave\LayeredNavigation\Helper\Url')
            ->disableOriginalConstructor()
            ->getMock();

        $scopeConfigReflector = new \ReflectionProperty('\Ewave\LayeredNavigation\Helper\Url', 'scopeConfig');
        $scopeConfigReflector->setAccessible(true);
        $scopeConfigReflector->setValue($this->_urlHelper, $this->_scopeConfig);

        $reflector = new \ReflectionClass('\Ewave\LayeredNavigation\Helper\UrlParser');
        $this->_urlParser = $reflector->newInstanceWithoutConstructor();

        $this->_seoHelper = $this->getMockBuilder('\Ewave\LayeredNavigation\Helper\Data')
            ->setMethods(['getOptionsSeoData', 'parseRequestUri'])
            ->disableOriginalConstructor()
            ->getMock();

        $seoHelperReflector = new \ReflectionProperty('\Ewave\LayeredNavigation\Helper\UrlParser', 'seoHelper');
        $seoHelperReflector->setAccessible(true);
        $seoHelperReflector->setValue($this->_urlParser, $this->_seoHelper);

        $this->_objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager($this);
        $this->_router = $this->_objectManager->getObject('\Ewave\LayeredNavigation\Controller\Router', [
        ...
        $requestReflector = new \ReflectionProperty('\Ewave\LayeredNavigation\Helper\Url', '_request');
    ...
    public function testMatchSeoAttribute()
    {
        ...
        $this->_scopeConfig->expects($this->any())
            ->method('isSetFlag')
            ->with('ewave_layerednavigation/url/mode', 'website')
            ->willReturn(true);
