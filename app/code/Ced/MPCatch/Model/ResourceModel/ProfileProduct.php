...
namespace Ced\MPCatch\Model\ResourceModel;
...
class ProfileProduct extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('mpcatch_profile_products', 'id');
    }
    ...
    public function deleteFromProfile($productId)
    {
        ...
        $condition = "{$this->getTable('mpcatch_profile_products')}.product_id = " . $dbh->quote($productId);
        //. " AND {$this->getTable('mpcatch/profileproducts')}.profile_id = " . $dbh->quote($profileId);
        $dbh->delete($this->getTable('mpcatch_profile_products'), $condition);
        return $this;
    }
    ...
    public function deleteProducts($productIds)
    {
        ...
        $dbh = $this->getConnection();
        $condition =
            "{$this->getTable('mpcatch_profile_products')}.product_id IN (" . implode(',', $productIds) . ")";
        $dbh->delete($this->getTable('mpcatch_profile_products'), $condition);
        return $this;
    }
    ...
    public function addProducts($productIds, $profileId)
    {
        ...
        $dbh = $this->getConnection();
        $dbh->insertMultiple($this->getTable('mpcatch_profile_products'), $data);
        return $this;
    }
    ...
    {
        if ($productId > 0) {
            $profileTable = $this->getTable('mpcatch_profile_products');
            $productProfile = \Magento\Framework\App\ObjectManager::getInstance()
                ->get('Ced\MPCatch\Model\ProfileProduct')->loadByField('profile_id', $profileId);
