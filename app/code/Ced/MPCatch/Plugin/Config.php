...
class Config
{
    ...
    public function __construct(
        ...
        \Ced\MPCatch\Helper\Config $mpcatchConfig
    ) {
    ...
    public function afterSave(
        ...
        if (isset($configPost['section']) and $configPost['section'] == 'mpcatch_config') {
            $enabled = $this->config->isEnabled();
            if ($enabled) {
                $response = $this->config->validate();
                if ($response) {
                    $this->messageManager->addSuccessMessage('Catch credentials are valid.');
                    $this->scopeConfigResource->saveConfig(
                        'mpcatch_config/mpcatch_setting/valid',
                        '1',
                        'default',
                        0
                    );
                } else {
                    $this->messageManager->addErrorMessage('Catch credentials are invalid.');
                    $this->scopeConfigResource->saveConfig(
                        'mpcatch_config/mpcatch_setting/valid',
                        '0',
                        'default',
