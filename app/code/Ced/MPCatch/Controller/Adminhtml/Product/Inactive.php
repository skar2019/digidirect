...
class Inactive extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if (isset($batch_id)) {
            ...
            $response = $this->mpcatch->updatePriceInventory($productIds[$batch_id], false, true);
            if (isset($productIds[$batch_id]) && $response) {
                $attrData = array( 'catch_exclude_from_sync' => '1' );
                $storeId = 0;
        ...
        if (count($productIds) == self::CHUNK_SIZE) {
            $response = $this->mpcatch->updatePriceInventory($productIds, false, true);
            if ($response) {
                $attrData = array( 'catch_exclude_from_sync' => '1' );
                $storeId = 0;
        ...
        $resultPage->getConfig()->getTitle()->prepend(__('Inactive On Catch'));
        return $resultPage;
    }
}
