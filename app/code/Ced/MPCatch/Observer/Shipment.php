...
class Shipment implements \Magento\Framework\Event\ObserverInterface
{
	...
	public function execute(\Magento\Framework\Event\Observer $observer)
	{
		...
		if($observer->getEvent()->getTrack()) {
			...
			if($order_id) {
				try{
					$bo_order = $this->objectManager->get('Ced\MPCatch\Model\Orders')->load($order_id,'magento_order_id');
					if($bo_order && $bo_order->getMpcatchOrderId()) {
						...
						$args = ['TrackingNumber' => $tracking_number,'ShippingProvider' => strtoupper($carrier_code),'order_id' => $order_id, 'MPCatchOrderID' => $bo_order->getMpcatchOrderId(), 'ShippingProviderName' => strtolower($carrier_name) ];
