...
namespace Ced\MPCatch\Ui\DataProvider\Profile;
...
class Products extends \Magento\Ui\DataProvider\AbstractDataProvider
{
    ...
    )
    {
        ...
        $profileId = $this->request->getParam('mpcatch_profile_id');
        ...
        if (isset($profileId) and !empty($profileId)) {
            foreach ($bookmarks as $bookmark) {
                if ($bookmark->getIdentifier() == 'current') {
                    ...
                    $configValue['current']['filters']['applied']['mpcatch_profile_id'] = $profileId;
            ...
            foreach ($bookmarks as $bookmark) {
                if ($bookmark->getIdentifier() == 'current') {
                    $configValue = $bookmark->getConfig();
                    if (isset($configValue['current']['filters']['applied']['mpcatch_profile_id'])) {
                        unset($configValue['current']['filters']['applied']['mpcatch_profile_id']);
                    }
                    $bookmark->setConfig(json_encode($configValue));
                    $bookmark->save();
                }
