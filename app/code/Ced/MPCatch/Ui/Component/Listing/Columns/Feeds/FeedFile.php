...
class FeedFile extends Column
{
     ...
     * Url path 
     */
    const URL_PATH_RESEND = 'mpcatch/feeds/resend';
    ...
    public function getFileLink($filePath)
    {
        ...
        if (isset($filePath) and !empty($filePath)) {
            ...
            $file = $this->dl->getPath('media') . "/mpcatch/feed/" . $fileName;
            if ($this->file->fileExists($file)) {
                $url = $this->urlBuilder->getBaseUrl() . "pub/media/mpcatch/feed/" . $fileName;
            } else {
                if ($this->file->fileExists($filePath)) {
                    $cpDir = $this->dl->getPath('media') . "/mpcatch/feed/";
