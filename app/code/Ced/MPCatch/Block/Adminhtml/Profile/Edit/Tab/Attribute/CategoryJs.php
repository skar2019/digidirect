...
namespace Ced\MPCatch\Block\Adminhtml\Profile\Edit\Tab\Attribute;
...
class CategoryJs extends Widget implements RendererInterface
{
     ...
     * @param \CatchSdk\Product                     $mpcatch
     * @param array                                   $data
     */
    public function __construct(
        ...
        $this->mpcatch = $mpcatch->create(['config' => $config->getApiConfig()]);
        parent::__construct($context, $data);
    }

    /**
     * Get current level of MPCatch category
     *
     * @param  integer $level
     * @return string
     */
    public function getLevel($level)
    {
        ...
        $location = $objectManager->get('Magento\Framework\App\Config\ScopeConfigInterface')
            ->getValue('mpcatch_config/mpcatch_setting/location');
        $locationList = $objectManager->get('Ced\MPCatch\Model\Config\Location')->toOptionArray();
        ...
        $folderPath = $mediaDirectory->getAbsolutePath('code/Ced/MPCatch/Setup/json/');
        $path = $folderPath . $locationName . '/categoryLevel-' . $level . '.json';
        $rootlevel = $objectManager->get('Ced\MPCatch\Helper\Data')->loadFile($path);
