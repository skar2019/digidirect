...
class AttributeMapping extends \Magento\Backend\Block\Template
{
     ...
     * @var string
     */
    public $_template = 'Ced_MPCatch::profile/attribute/attributes.phtml';
    ...
    public function getAddButtonHtml()
    {
        $button = $this->getLayout()->createBlock(
            ...
            [
                ...
                'onclick' => 'return mpcatchAttributeControl.addItem()',
                'class' => 'add'
            ]
        );

        $button->setName('mpcatch_add_attribute_mapping_button');
        return $button->toHtml();
    }

    public function getMPCatchAttributes()
    {
        // For AJAX
        $this->_mpcatchAttribute = $this->getAttributes();

        
        if (isset($this->_mpcatchAttribute) and !empty($this->_mpcatchAttribute)) {
            return $this->_mpcatchAttribute;
        }
    ...
    {
        $data = $this->_mpcatchAttribute[0]['value'];
        $reqAttrCodes = array_keys($data);
        $optData = $this->_mpcatchAttribute[1]['value'];
