<?php
return [
    'scopes' => [
        'websites' => [
            'admin' => [
                'website_id' => '0',
                'code' => 'admin',
                'name' => 'Admin',
                'sort_order' => '0',
                'default_group_id' => '0',
                'is_default' => '0'
            ],
            'digi_website_au' => [
                'website_id' => '1',
                'code' => 'digi_website_au',
                'name' => 'digiDirect Au',
                'sort_order' => '0',
                'default_group_id' => '1',
                'is_default' => '1'
            ]
        ],
        'groups' => [
            [
                'group_id' => '0',
                'website_id' => '0',
                'name' => 'Default',
                'root_category_id' => '0',
                'default_store_id' => '0',
                'code' => 'default'
            ],
            [
                'group_id' => '1',
                'website_id' => '1',
                'name' => 'digiDirect AU',
                'root_category_id' => '2',
                'default_store_id' => '1',
                'code' => 'digi_store_au'
            ]
        ],
        'stores' => [
            'admin' => [
                'store_id' => '0',
                'code' => 'admin',
                'website_id' => '0',
                'group_id' => '0',
                'name' => 'Admin',
                'sort_order' => '0',
                'is_active' => '1'
            ],
            'digi_store_view_au' => [
                'store_id' => '1',
                'code' => 'digi_store_view_au',
                'website_id' => '1',
                'group_id' => '1',
                'name' => 'digiDirect AU',
                'sort_order' => '0',
                'is_active' => '1'
            ]
        ]
    ],
    'system' => [
        'default' => [
            'general' => [
                'locale' => [
                    'code' => 'en_AU'
                ]
            ],
            'dev' => [
                'static' => [
                    'sign' => '1'
                ],
                'front_end_development_workflow' => [
                    'type' => 'server_side_compilation'
                ],
                'template' => [
                    'minify_html' => '1',
                    'allow_symlink' => '0'
                ],
                'js' => [
                    'merge_files' => '1',
                    'minify_files' => '1',
                    'minify_exclude' => [
                        'tiny_mce' => '/tiny_mce/',
                        'authorizenet_acceptjs' => '\\.authorize\\.net/v1/Accept'
                    ],
                    'session_storage_logging' => '0',
                    'translate_strategy' => 'dictionary',
                    'enable_js_bundling' => '0'
                ],
                'css' => [
                    'minify_files' => '1',
                    'minify_exclude' => [
                        'tiny_mce' => '/tiny_mce/'
                    ],
                    'merge_css_files' => '1'
                ]
            ]
        ],
        'stores' => [

        ],
        'websites' => [

        ]
    ],
    'modules' => [
        'Magento_Store' => 1,
        'Magento_Directory' => 1,
        'Magento_Theme' => 1,
        'Apptrian_ImageOptimizer' => 1,
        'Criteo_OneTag' => 1,
        'Ced_MPCatch' => 1,
        'Digi_Minicart' => 1,
        'Digi_Order' => 1,
        'Digi_Qantas' => 1,
        'Magento_Variable' => 1,
        'Magento_Eav' => 1,
        'Magento_Backend' => 1,
        'Magento_AdminNotification' => 1,
        'Magento_Indexer' => 1,
        'Magento_Customer' => 1,
        'Magento_CatalogImportExport' => 1,
        'Magento_Rule' => 1,
        'Magento_Authorization' => 1,
        'Magento_Config' => 1,
        'Magento_Cms' => 1,
        'Magento_Catalog' => 1,
        'Magento_Cron' => 1,
        'Magento_Quote' => 1,
        'Magento_CatalogRule' => 1,
        'Magento_SalesSequence' => 1,
        'Ewave_InfiniteScroll' => 1,
        'Magento_Search' => 1,
        'Magento_Payment' => 1,
        'Ewave_CmsUpgrade' => 1,
        'Magento_CatalogInventory' => 1,
        'Magento_Sales' => 1,
        'Magento_Checkout' => 1,
        'Ewave_Localization' => 1,
        'Magento_Msrp' => 1,
        'Magento_ConfigurableProduct' => 1,
        'Magento_Widget' => 1,
        'Ewave_Email' => 1,
        'Ewave_Utilities' => 1,
        'Magento_Vault' => 1,
        'Magento_InventoryApi' => 1,
        'Magento_CatalogRuleConfigurable' => 1,
        'Ewave_ProductCalculator' => 1,
        'Ewave_ExtendedMiniCart' => 1,
        'Ewave_ExtendedShippingRates' => 1,
        'Ewave_ExtendedShippingRatesLocalization' => 1,
        'Ewave_ShippingAvailabilityCheck' => 1,
        'Ewave_Faq' => 1,
        'Ewave_Feed' => 1,
        'Magento_SalesRule' => 1,
        'Ewave_GiftCardImage' => 1,
        'Ewave_Googleapi' => 1,
        'Magento_Captcha' => 1,
        'Magento_Inventory' => 1,
        'Magento_LayeredNavigation' => 1,
        'Ewave_Locator' => 1,
        'Ewave_AbstractEntity' => 1,
        'Magento_RequireJs' => 1,
        'Ewave_AI' => 1,
        'Ewave_MagentoFixes' => 1,
        'Ewave_Pronto' => 1,
        'Ewave_MyStoreWidget' => 1,
        'Ewave_Collect' => 1,
        'Magento_User' => 1,
        'Ewave_NavigationCMSUpgrade' => 1,
        'Magento_Newsletter' => 1,
        'Ewave_OrderImages' => 1,
        'Magento_ProductAlert' => 1,
        'Ewave_PreOrder' => 1,
        'Ewave_OutOfStockNotif' => 1,
        'Ewave_ProductAttachment' => 1,
        'Ewave_ExtendedCatalogPriceRuleProductCalculator' => 1,
        'Ewave_ProductFilter' => 1,
        'Ewave_ProductMultiView' => 1,
        'Magento_MediaStorage' => 1,
        'Ewave_ProductPriority' => 1,
        'Magento_CatalogSearch' => 1,
        'Magento_Rma' => 1,
        'Ewave_QuickView' => 1,
        'Magento_TargetRule' => 1,
        'Ewave_SEO' => 1,
        'Ewave_Security' => 1,
        'Ewave_ExtendedShippingRatesShippingAvailability' => 1,
        'Ewave_ShippingAvailabilityCheckCollect' => 1,
        'Ewave_SingleCheckoutButton' => 1,
        'Ewave_SitemapWidget' => 1,
        'Ewave_SocialSharing' => 1,
        'Ewave_Store' => 1,
        'Ewave_CollectAbstractEntity' => 1,
        'Ewave_StoreLocatorInfiniteScroll' => 1,
        'Ewave_StyleGuide' => 1,
        'Ewave_CheckoutFields' => 1,
        'Magento_CustomerBalance' => 1,
        'Magento_CacheInvalidate' => 1,
        'Magento_Paypal' => 1,
        'Klarna_Core' => 0,
        'Klarna_Ordermanagement' => 0,
        'Klevu_Search' => 1,
        'Klevu_Content' => 1,
        'Klevu_Addtocart' => 1,
        'LiveChat_LiveChat' => 1,
        'MSP_ReCaptcha' => 1,
        'MSP_TwoFactorAuth' => 1,
        'Magento_AdminGws' => 1,
        'Amazon_Core' => 0,
        'Magento_AdvancedCatalog' => 1,
        'Magento_AdvancedCheckout' => 1,
        'Magento_AdvancedPricingImportExport' => 1,
        'Magento_AdvancedRule' => 1,
        'Magento_AdvancedSalesRule' => 1,
        'Magento_AdvancedSearch' => 1,
        'Magento_Amqp' => 1,
        'Magento_Security' => 1,
        'Magento_AsynchronousOperations' => 1,
        'Ewave_AbstractAttributes' => 1,
        'Magento_Authorizenet' => 0,
        'Magento_AuthorizenetAcceptjs' => 0,
        'Magento_Bundle' => 1,
        'Magento_Backup' => 1,
        'Magento_VersionsCms' => 1,
        'Magento_CustomerCustomAttributes' => 1,
        'Magento_CatalogWidget' => 1,
        'Magento_Banner' => 1,
        'Magento_Braintree' => 1,
        'Magento_CustomerSegment' => 1,
        'Magento_GraphQl' => 0,
        'Magento_BundleImportExport' => 1,
        'Magento_BundleImportExportStaging' => 1,
        'Magento_CatalogUrlRewrite' => 1,
        'Magento_PageCache' => 1,
        'Magento_Wishlist' => 1,
        'Amazon_Login' => 0,
        'Magento_Integration' => 1,
        'Magento_CatalogEvent' => 1,
        'Magento_EavGraphQl' => 0,
        'Magento_GiftCardAccount' => 1,
        'Magento_CatalogImportExportStaging' => 1,
        'Amazon_Payment' => 0,
        'Magento_CatalogInventoryGraphQl' => 0,
        'Magento_Ui' => 1,
        'Magento_CatalogPageBuilderAnalytics' => 0,
        'Magento_CatalogPageBuilderAnalyticsStaging' => 0,
        'Magento_WebsiteRestriction' => 1,
        'Magento_InventoryCatalog' => 1,
        'Magento_Staging' => 1,
        'Magento_CatalogRuleStaging' => 1,
        'Magento_LayeredNavigationStaging' => 1,
        'Magento_Downloadable' => 1,
        'Magento_GiftCard' => 1,
        'Magento_GiftRegistry' => 1,
        'Magento_StoreGraphQl' => 0,
        'Magento_UrlRewrite' => 1,
        'Magento_BannerCustomerSegment' => 1,
        'Magento_Shipping' => 1,
        'Magento_CheckoutAgreements' => 1,
        'Magento_CheckoutStaging' => 1,
        'Magento_CloudComponents' => 1,
        'Ewave_AbstractEntityPopup' => 1,
        'Magento_CmsGraphQl' => 0,
        'Magento_CmsPageBuilderAnalytics' => 0,
        'Magento_CmsPageBuilderAnalyticsStaging' => 0,
        'Magento_CmsStaging' => 1,
        'Magento_CmsUrlRewrite' => 1,
        'Magento_CmsUrlRewriteGraphQl' => 0,
        'Ewave_CollectAbstractEntityMSI' => 1,
        'Magento_ConfigurableImportExport' => 1,
        'Ewave_AbstractGiftCard' => 1,
        'Magento_CatalogGraphQl' => 0,
        'Magento_ConfigurableProductSales' => 1,
        'Magento_Reports' => 1,
        'Magento_Contact' => 1,
        'Magento_Cookie' => 1,
        'Magento_InventorySales' => 1,
        'Magento_CurrencySymbol' => 1,
        'Magento_CustomAttributeManagement' => 1,
        'Digidirect_Catalog' => 1,
        'Magento_Analytics' => 1,
        'Ewave_AbstractGiftCardLogger' => 1,
        'Magento_PageBuilder' => 0,
        'Magento_CustomerFinance' => 1,
        'Magento_CustomerGraphQl' => 0,
        'Magento_CustomerImportExport' => 1,
        'Magento_Email' => 1,
        'Magento_Cybersource' => 0,
        'Magento_Deploy' => 1,
        'Magento_Developer' => 1,
        'Magento_Dhl' => 0,
        'Magento_GroupedProduct' => 1,
        'Magento_DirectoryGraphQl' => 0,
        'Magento_Tax' => 1,
        'Magento_DownloadableGraphQl' => 0,
        'Magento_ImportExport' => 1,
        'Magento_Weee' => 1,
        'Digidirect_Customer' => 1,
        'Magento_BundleGraphQl' => 0,
        'Magento_Elasticsearch' => 1,
        'Magento_Elasticsearch6' => 1,
        'Magento_Review' => 1,
        'Magento_EncryptionKey' => 1,
        'Magento_Enterprise' => 1,
        'Magento_Eway' => 0,
        'Magento_Fedex' => 0,
        'Magento_CatalogStaging' => 1,
        'Ewave_Vii' => 1,
        'Magento_GiftCardGraphQl' => 0,
        'Magento_GiftCardImportExport' => 1,
        'Magento_GiftCardStaging' => 1,
        'Magento_GiftMessage' => 1,
        'Magento_GiftMessageStaging' => 1,
        'Magento_ConfigurableProductStaging' => 1,
        'Magento_GiftWrapping' => 1,
        'Magento_GiftWrappingStaging' => 1,
        'Magento_GoogleAdwords' => 1,
        'Magento_GoogleAnalytics' => 1,
        'Magento_GoogleOptimizer' => 1,
        'Magento_GoogleOptimizerStaging' => 1,
        'Magento_GoogleTagManager' => 1,
        'Magento_UrlRewriteGraphQl' => 0,
        'Magento_GroupedCatalogInventory' => 1,
        'Magento_GroupedImportExport' => 1,
        'Magento_SendFriend' => 1,
        'Magento_GroupedProductGraphQl' => 0,
        'Magento_GroupedProductStaging' => 1,
        'Magento_DownloadableImportExport' => 1,
        'Ewave_AddressVerification' => 1,
        'Magento_InstantPurchase' => 1,
        'Magento_CatalogAnalytics' => 1,
        'Ewave_MSI' => 1,
        'Magento_InventoryAdminUi' => 1,
        'Ewave_ExtendedCartPriceRulesMSI' => 1,
        'Magento_InventoryBundleProduct' => 1,
        'Magento_InventoryBundleProductAdminUi' => 1,
        'Magento_InventoryIndexer' => 1,
        'Ewave_InvoiceIncrementId' => 1,
        'Magento_InventoryCatalogAdminUi' => 1,
        'Magento_InventoryCatalogApi' => 1,
        'Magento_InventoryCatalogSearch' => 1,
        'Magento_InventoryConfigurableProduct' => 1,
        'Magento_InventoryConfigurableProductAdminUi' => 1,
        'Magento_InventoryConfigurableProductIndexer' => 1,
        'Magento_InventoryConfiguration' => 1,
        'Magento_InventoryConfigurationApi' => 1,
        'Magento_InventoryDistanceBasedSourceSelection' => 1,
        'Magento_InventoryDistanceBasedSourceSelectionAdminUi' => 1,
        'Magento_InventoryDistanceBasedSourceSelectionApi' => 1,
        'Magento_InventoryElasticsearch' => 1,
        'Magento_InventoryGroupedProduct' => 1,
        'Magento_InventoryGroupedProductAdminUi' => 1,
        'Magento_InventoryGroupedProductIndexer' => 1,
        'Magento_InventoryImportExport' => 1,
        'Magento_InventoryCache' => 1,
        'Magento_InventoryLowQuantityNotification' => 1,
        'Magento_InventoryLowQuantityNotificationAdminUi' => 1,
        'Magento_InventoryLowQuantityNotificationApi' => 1,
        'Magento_InventoryMultiDimensionalIndexerApi' => 1,
        'Magento_InventoryProductAlert' => 1,
        'Magento_InventoryReservations' => 1,
        'Magento_InventoryReservationsApi' => 1,
        'Ewave_ProntoDigi' => 1,
        'Magento_InventorySalesAdminUi' => 1,
        'Magento_InventorySalesApi' => 1,
        'Magento_InventorySalesFrontendUi' => 1,
        'Magento_InventorySetupFixtureGenerator' => 1,
        'Magento_InventoryShipping' => 1,
        'Magento_InventoryShippingAdminUi' => 1,
        'Magento_InventorySourceDeductionApi' => 1,
        'Magento_InventorySourceSelection' => 1,
        'Magento_InventorySourceSelectionApi' => 1,
        'Magento_Invitation' => 1,
        'Ewave_LayeredNavigation' => 1,
        'Ewave_CategoryFilter' => 1,
        'Magento_Logging' => 1,
        'Magento_Marketplace' => 0,
        'Magento_Swatches' => 1,
        'Magento_MessageQueue' => 1,
        'Ewave_AISales' => 1,
        'Magento_MsrpConfigurableProduct' => 1,
        'Magento_MsrpGroupedProduct' => 1,
        'Magento_MsrpStaging' => 1,
        'Magento_MultipleWishlist' => 1,
        'Magento_Multishipping' => 1,
        'Magento_MysqlMq' => 1,
        'Magento_NewRelicReporting' => 1,
        'Ewave_Newsletter' => 1,
        'Magento_OfflinePayments' => 1,
        'Magento_OfflineShipping' => 1,
        'Magento_BannerPageBuilder' => 0,
        'Magento_PageBuilderAnalytics' => 0,
        'Fastly_Cdn' => 1,
        'Ewave_StoreLocator' => 1,
        'Magento_PaymentStaging' => 1,
        'Ewave_ExtendedCartPriceRules' => 1,
        'Magento_PaypalOnBoarding' => 1,
        'Magento_Persistent' => 1,
        'Magento_PersistentHistory' => 1,
        'Magento_PricePermissions' => 1,
        'Ewave_OutOfStockNotifPreOrder' => 1,
        'Magento_ProductVideo' => 1,
        'Magento_ProductVideoStaging' => 1,
        'Magento_PromotionPermissions' => 1,
        'Ewave_Blog' => 1,
        'Magento_QuoteAnalytics' => 1,
        'Magento_QuoteGraphQl' => 0,
        'Magento_ReleaseNotification' => 1,
        'Magento_Reminder' => 1,
        'Magento_CatalogStagingPageBuilder' => 0,
        'Ess_M2ePro' => 1,
        'Magento_ResourceConnections' => 1,
        'Magento_Reward' => 1,
        'Magento_ReviewAnalytics' => 1,
        'Magento_ReviewStaging' => 1,
        'Dotdigitalgroup_Email' => 1,
        'Magento_RewardGraphQl' => 0,
        'Magento_SalesRuleStaging' => 1,
        'Ewave_Digi' => 1,
        'Magento_RmaGraphQl' => 0,
        'Magento_RmaStaging' => 1,
        'Magento_Robots' => 1,
        'Magento_Rss' => 1,
        'Ewave_Banner' => 1,
        'Ewave_CollectStoreLocator' => 1,
        'Magento_SalesAnalytics' => 1,
        'Magento_Signifyd' => 1,
        'Magento_SalesGraphQl' => 0,
        'Magento_SalesInventory' => 1,
        'Ewave_FreeGift' => 1,
        'Magento_RewardStaging' => 1,
        'Ewave_MyStoreWidgetCollect' => 1,
        'Magento_SampleData' => 1,
        'Magento_ScalableCheckout' => 1,
        'Magento_ScalableInventory' => 1,
        'Magento_ScalableOms' => 1,
        'Magento_ScheduledImportExport' => 1,
        'Ewave_Navigation' => 1,
        'Magento_SearchStaging' => 1,
        'Magento_CustomerAnalytics' => 1,
        'Dotdigitalgroup_Enterprise' => 1,
        'Magento_SendFriendGraphQl' => 0,
        'Ewave_M2ePro' => 1,
        'Magento_SalesArchive' => 1,
        'Magento_Sitemap' => 1,
        'Magento_CatalogInventoryStaging' => 1,
        'Magento_StagingPageBuilder' => 0,
        'Ewave_CriteoOneTag' => 1,
        'Magento_ConfigurableProductGraphQl' => 0,
        'Magento_Support' => 1,
        'Magento_Webapi' => 1,
        'Magento_SwaggerWebapi' => 1,
        'Magento_SwaggerWebapiAsync' => 1,
        'Ewave_ProductOverlay' => 1,
        'Magento_SwatchesGraphQl' => 0,
        'Magento_SwatchesLayeredNavigation' => 1,
        'Ewave_RelatedProduct' => 1,
        'Magento_DownloadableStaging' => 1,
        'Magento_TaxGraphQl' => 0,
        'Magento_TaxImportExport' => 1,
        'Ewave_CustomSitemapLinks' => 1,
        'Magento_ThemeGraphQl' => 0,
        'Magento_Tinymce3' => 1,
        'Magento_Tinymce3Banner' => 1,
        'Magento_Translation' => 1,
        'Ewave_ExtendedCatalogPriceRule' => 1,
        'Magento_Ups' => 0,
        'Magento_CatalogUrlRewriteStaging' => 1,
        'Magento_CatalogUrlRewriteGraphQl' => 0,
        'Ewave_AbstractAttributesNavigation' => 1,
        'Magento_Usps' => 0,
        'Ewave_Migration' => 1,
        'Klarna_Kp' => 0,
        'Magento_Version' => 0,
        'Magento_BannerPageBuilderAnalytics' => 0,
        'Magento_VisualMerchandiser' => 1,
        'Magento_Swagger' => 1,
        'Magento_WebapiAsync' => 1,
        'Magento_WebapiSecurity' => 1,
        'Magento_CatalogPermissions' => 1,
        'Magento_BundleStaging' => 1,
        'Magento_WeeeGraphQl' => 0,
        'Magento_WeeeStaging' => 1,
        'Ewave_Emailblocker' => 1,
        'Ewave_BlogInfiniteScroll' => 1,
        'Magento_WishlistAnalytics' => 1,
        'Magento_WishlistGraphQl' => 0,
        'Magento_Worldpay' => 0,
        'Studio19_Variants' => 1,
        'Temando_Shipping' => 0,
        'Vertex_Tax' => 1,
        'ZipMoney_ZipMoneyPayment' => 1
    ],
    'admin_user' => [
        'locale' => [
            'code' => [
                'en_AU'
            ]
        ]
    ]
];
