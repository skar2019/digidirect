  ...
  $(document).on('click', '#custom-alert-close', function () {
    ...
    $(function () {
        ...
        function updateCartAjax($input, newQty) {
          ...
          if (!itemIdMatch) {
            console.error('❌ cart-qty: cannot extract item ID from', $input.attr('name'))
            return
          }

          const itemId = itemIdMatch[1]
          console.log(`🧩 cart-qty: updating item ${itemId} → qty ${newQty}`)
          ...
          $.ajax({
            ...
            success: function (res) {
              console.log('✅ cart-qty: update success', res)

              // 🔄 Refresh minicart and totals
              customerData.reload(['cart', 'checkout-data'], true)

              // Reload totals section
              $('.cart-totals').load(window.location.href + ' .cart-totals > *')

              // Reload subtotal cell for this item
              const $row = $input.closest('tbody.cart.item')
              if ($row.length) {
            ...
            error: function (xhr, status, err) {
              console.error('❌ cart-qty: AJAX update failed', status, err)
            },
            complete: function () {
        ...
        $(document).on('click', '.qty-increase', function (e) {
          ...
          if (!$input.length) return console.warn('⚠️ cart-qty: no qty input found (increase)')
        ...
        $(document).on('click', '.qty-decrease', function (e) {
          ...
          const $input = $(this).closest('.qty-buttons').find('input.input-text.qty')
          if (!$input.length) return console.warn('⚠️ cart-qty: no qty input found (decrease)')
