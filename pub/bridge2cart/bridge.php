          ...
          try {
            ...
            foreach ($res as $country => $providers) {
              foreach ($providers as $providerName => $url) {
                $data[sanitize_title($providerName)] = array('name' => $providerName, 'country' => $country, 'url' => $url);
              }
            }
...
class M1_Bridge_Action_Update
{
  ...
  public function perform(M1_Bridge $bridge)
  {
    ...
    if (!(is_writeable(dirname(__DIR__)) && is_writeable(__FILE__))) {
      $response->code = 1;
...
class M1_Config_Adapter_Wordpress extends M1_Config_Adapter
{
  ...
  public function __construct()
  {
    ...
    $constants = M1_Bridge::parseDefinedConstants(
      M1_Bridge::removeComments($config), 'DB_NAME|DB_USER|DB_PASSWORD|DB_HOST|UPLOADS'
    );
    ...
    while ($count++ < 2) {
      ...
      $cartPluginsNetwork = $this->getCartVersionFromDb("meta_value", "sitemeta", "meta_key = 'active_sitewide_plugins'");
      if ($cartPluginsNetwork) {
        $cartPluginsNetwork = unserialize($cartPluginsNetwork);
        $cartPlugins = array_keys($cartPluginsNetwork);
      } else {
        break;
      }
    }
...
class M1_Config_Adapter_Magento1212 extends M1_Config_Adapter
{
  ...
  protected function _magento2()
  {
    ...
    if (file_exists(M1_STORE_BASE_DIR . 'composer.json')) {
      ...
      $ee = ($json['name'] == 'magento/project-enterprise-edition' ? 'EE.' : '');
      $this->cartVars['dbVersion'] = $ee . $json['version'];
    } else {
