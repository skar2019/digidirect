...
class Library extends Feed
{
    ...
    public function execute()
    {
        if ($this->getRequest()->getParam('pattern')) {
            $content = $this->layout->createBlock('Ewave\Feed\Block\Adminhtml\Feed\Library')
                ->setPattern($this->getRequest()->getParam('pattern'))
                ->setTemplate('Ewave_Feed::feed/library/preview.phtml')
                ->toHtml();
        } else {
            $content = $this->layout->createBlock('Ewave\Feed\Block\Adminhtml\Feed\Library')
                ->setTemplate('Ewave_Feed::feed/library.phtml')
                ->toHtml();
        }
