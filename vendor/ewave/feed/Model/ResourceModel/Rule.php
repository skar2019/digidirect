...
class Rule extends AbstractDb
{
    ...
    {
        $this->_init('ewave_feed_rule', 'rule_id');
    }
    ...
    public function getFeedIds(AbstractModel $object)
    {
        ...
        $select = $connection->select()
            ->from($this->getTable('ewave_feed_rule_feed'), ['feed_id'])
    ...
    {
        /** @var \Ewave\Feed\Model\Rule $object */
        $connection = $this->getConnection();
    ...
    {
        $connection = $this->getConnection();
        $connection->delete($this->getTable('ewave_feed_rule_product'), ['rule_id = ?' => $ruleId]);
    ...
    public function saveProductIds($ruleId, array $productIds = [])
    {
        ...
        $connection->insertOnDuplicate($this->getTable('ewave_feed_rule_product'), $data);
