<?php
namespace Ewave\Faq\Model\ResourceModel\Faq;
...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    {
        $this->_init('Ewave\Faq\Model\Faq', 'Ewave\Faq\Model\ResourceModel\Faq');
    }
    ...
    {
        $tableName = 'ewave_faq_tag_relation';
        $linkField = 'faq_id';
        $linkedIds = $this->getColumnValues('entity_id');
        if (!empty($linkedIds)) {
            $connection = $this->getConnection();
            $select = $connection->select()
                ->from(['ewave_tag_relation' => $this->getTable($tableName)], [])
                ->joinLeft(
                    ['tag' => $this->getTable('ewave_faq_tag')],
                    'ewave_tag_relation.tag_id = tag.entity_id',
                    ['tag_name' => 'title']
                )
                ->where('ewave_tag_relation.' . $linkField . ' IN (?)', $linkedIds);
    ...
    {
        $tableName = 'ewave_faq_category_relation';
        $linkField = 'faq_id';
        $linkedIds = $this->getColumnValues('entity_id');
        if (!empty($linkedIds)) {
            $connection = $this->getConnection();
            $select = $connection->select()
                ...
                ->joinLeft('ewave_faq_category as c', 'c.entity_id = rel.category_id', ['title'])
    ...
    public function addFilterByCategory($ids)
    {
        ...
        $this->getSelect()
            ->joinInner(
                ['rel' => $this->getTable('ewave_faq_category_relation')],
    ...
    public function addFilterByTag($ids)
    {
        ...
        $this->getSelect()
            ->joinInner(['rel' => $this->getTable('ewave_faq_tag_relation')], 'rel.faq_id = main_table.entity_id', [])
