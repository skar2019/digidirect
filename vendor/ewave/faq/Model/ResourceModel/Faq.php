...
use Ewave\Faq\Model\Faq as FaqItem;
...
{
    const EWAVE_FAQ_FAQ_ITEM_INFO_TABLE = 'ewave_faq';
    ...
    {
        $this->_init('ewave_faq', 'entity_id');
    }
    ...
    public function lookupCategoryIds($faqId)
    {
        ...
        $select = $connection->select()
            ->from(['fcr' => $this->getTable('ewave_faq_category_relation')], 'category_id')
            ->join(
                ['f' => $this->getMainTable()],
    ...
    public function lookupTags($faqId)
    {
        ...
        $linkField = $entityMetadata->getLinkField();
        $select = $connection->select()
            ->from(['ftr' => $this->getTable('ewave_faq_tag_relation')], 'tag_id')
            ->join(
                ['f' => $this->getMainTable()],
                'ftr.faq_id = f.' . $linkField,
                []
            )
            ->join(['tag' => $this->getTable('ewave_faq_tag')], 'ftr.tag_id = tag.entity_id', ['tag_name' => 'title'])
            ->where('f.' . $entityMetadata->getIdentifierField() . ' = :faq_id');
