...
class InstallSchema implements InstallSchemaInterface
{
    ...
    const EWAVE_NAVIGATION_MENU_ITEM_TYPE_TABLE = 'ewave_navigation_menu_item_type';
    const EWAVE_NAVIGATION_MENU_CUSTOM_LINK_TYPE_TABLE = 'ewave_navigation_menu_custom_link_type';
    const EWAVE_NAVIGATION_MENU_CATEGORY_TYPE_TABLE = 'ewave_navigation_menu_category_type';
    const EWAVE_NAVIGATION_MENU_CMS_BLOCK_TYPE_TABLE = 'ewave_navigation_menu_cms_block_type';
    const EWAVE_NAVIGATION_MENU_SET_LINK = 'ewave_navigation_menu_set_link';
    const EWAVE_NAVIGATION_MENU_SET = 'ewave_navigation_menu_set';
    const EWAVE_NAVIGATION_MENU_ITEM_INFO_TABLE = 'ewave_navigation_menu_item_info';
    const EWAVE_NAVIGATION_MENU_SET_STORE = 'ewave_navigation_menu_set_store';
    ...
    protected function _addCategoryDeleteTrigger(SchemaSetupInterface $setup)
    {
        try {
            ...
            /**
             * @var $menuItemInfoTable 'ewave_navigation_menu_item_info' - table contains title and status etc
             */
            $menuItemInfoTable = $connection->getTableName('ewave_navigation_menu_item_info');

            /**
             * @var $menuItemCategoryTypeTable - table contains category id
             */
            $menuItemCategoryTypeTable = $connection->getTableName('ewave_navigation_menu_category_type');
    ...
    protected function _addCMSBlockDeleteTrigger(SchemaSetupInterface $setup)
    {
        try {
             ...
             */
            $menuItemCMSBlockTypeTable = $connection->getTableName('ewave_navigation_menu_cms_block_type');
    ...
    protected function _createMenuItemTypeTable(SchemaSetupInterface $installer)
    {
        $tableItemType = $installer->getConnection()
            ->newTable($installer->getTable(self::EWAVE_NAVIGATION_MENU_ITEM_TYPE_TABLE))
            ->addColumn(
                'type_id',
                ...
                'Ewave Navigation Item Types Table'
            );
    ...
    protected function _createCategoryTypeTable(SchemaSetupInterface $installer)
    {
        $tableCategoryType = $installer->getConnection()
            ->newTable($installer->getTable(self::EWAVE_NAVIGATION_MENU_CATEGORY_TYPE_TABLE))
            ->addColumn(
                'category_id',
            ...
            ->addColumn(
                ...
                $installer->getFkName(
                    self::EWAVE_NAVIGATION_MENU_CATEGORY_TYPE_TABLE,
                    'category_id',
                ...
                $installer->getFkName(self::EWAVE_NAVIGATION_MENU_CATEGORY_TYPE_TABLE, 'store_id', 'store', 'store_id'),
                'store_id',
                $installer->getTable('store'),
                'store_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            )->addForeignKey(
                $installer->getFkName(self::EWAVE_NAVIGATION_MENU_CATEGORY_TYPE_TABLE, 'menu_item_id',
                    self::EWAVE_NAVIGATION_MAIN_ENTITY_TABLE, 'entity_id'),
                'menu_item_id',
                $installer->getTable(self::EWAVE_NAVIGATION_MAIN_ENTITY_TABLE),
                'entity_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            )->setComment(
                'Ewave Navigation Category Type Table'
            );
    ...
    protected function _createCustomLinkTypeTable(SchemaSetupInterface $installer)
    {
        $tableCustomLinkType = $installer->getConnection()
            ->newTable($installer->getTable(self::EWAVE_NAVIGATION_MENU_CUSTOM_LINK_TYPE_TABLE))
            ->addColumn(
                'store_id',
                \Magento\Framework\DB\Ddl\Table::TYPE_SMALLINT,
                null,
                ...
                $installer->getFkName(
                    self::EWAVE_NAVIGATION_MENU_CUSTOM_LINK_TYPE_TABLE,
                    'store_id',
                    'store',
                    'store_id'
                ),
                ...
                'Ewave Navigation Custom Link Type Table'
            );
    ...
    protected function _createCmsBlockTypeTable(SchemaSetupInterface $installer)
    {
        $tableCmsBlockType = $installer->getConnection()
            ->newTable($installer->getTable(self::EWAVE_NAVIGATION_MENU_CMS_BLOCK_TYPE_TABLE))
            ->addColumn(
                'cms_block_id',
                \Magento\Framework\DB\Ddl\Table::TYPE_SMALLINT,
                null,
                ...
                $installer->getFkName(self::EWAVE_NAVIGATION_MENU_CMS_BLOCK_TYPE_TABLE, 'cms_block_id', 'cms_block',
                    'block_id'),
                'cms_block_id',
                ...
                $installer->getFkName(self::EWAVE_NAVIGATION_MENU_CMS_BLOCK_TYPE_TABLE, 'store_id', 'store',
                    'store_id'),
                'store_id',
                $installer->getTable('store'),
                'store_id',
                ...
                $installer->getFkName(self::EWAVE_NAVIGATION_MENU_CMS_BLOCK_TYPE_TABLE, 'menu_item_id',
                    self::EWAVE_NAVIGATION_MAIN_ENTITY_TABLE, 'entity_id'),
                'menu_item_id',
                $installer->getTable(self::EWAVE_NAVIGATION_MAIN_ENTITY_TABLE),
                'entity_id',
                ...
                'Ewave Navigation Cms Block Type Table'
            );
    ...
    protected function _createMainTable(SchemaSetupInterface $installer)
    {
        ...
        $tableEntity = $installer->getConnection()
            ->newTable($installer->getTable(self::EWAVE_NAVIGATION_MAIN_ENTITY_TABLE))
            ->addColumn(
                'entity_id',
                \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER,
                null,
                ...
                $installer->getFkName(self::EWAVE_NAVIGATION_MAIN_ENTITY_TABLE, 'type_id',
                    self::EWAVE_NAVIGATION_MENU_ITEM_TYPE_TABLE, 'type_id'),
                'type_id',
                $installer->getTable(self::EWAVE_NAVIGATION_MENU_ITEM_TYPE_TABLE),
                'type_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            )->setComment(
                'Ewave Navigation Table'
            );
    ...
    protected function _createMenuItemInfoTable(SchemaSetupInterface $installer)
    {
        $tableAttributes = $installer->getConnection()
            ->newTable($installer->getTable(self::EWAVE_NAVIGATION_MENU_ITEM_INFO_TABLE))
            ->addColumn(
                'id',
                \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER,
                null,
            ...
            ->addForeignKey(
                ...
                $installer->getFkName('ewave_navigation_attributes', 'store_id', 'store', 'store_id'),
                'store_id',
                $installer->getTable('store'),
                'store_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            )->setComment(
                'Ewave Navigation Attributes'
            );
    ...
    protected function _createMenuSetTable(SchemaSetupInterface $installer)
    {
        $tableSets = $installer->getConnection()
            ->newTable($installer->getTable(self::EWAVE_NAVIGATION_MENU_SET))
            ->addColumn(
                'set_id',
                \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER,
                null,
                ...
                $installer->getIdxName(
                    $installer->getTable(self::EWAVE_NAVIGATION_MENU_SET),
                    ['name', 'set_code'],
    ...
    protected function _createMenuSetMenuEntityLinkTable(SchemaSetupInterface $installer)
    {
        $tableNavigationSetLink = $installer->getConnection()
            ->newTable($installer->getTable(self::EWAVE_NAVIGATION_MENU_SET_LINK))
            ->addColumn(
                'menu_set_id',
                \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER,
                null,
                ...
                $installer->getFkName(self::EWAVE_NAVIGATION_MENU_SET_LINK, 'menu_set_id',
                    self::EWAVE_NAVIGATION_MENU_SET, 'set_id'),
                'menu_set_id',
                $installer->getTable(self::EWAVE_NAVIGATION_MENU_SET),
                'set_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            )->addForeignKey(
                $installer->getFkName(self::EWAVE_NAVIGATION_MENU_SET_LINK, 'menu_entity_id',
                    self::EWAVE_NAVIGATION_MAIN_ENTITY_TABLE, 'entity_id'),
                'menu_entity_id',
                $installer->getTable(self::EWAVE_NAVIGATION_MAIN_ENTITY_TABLE),
                'entity_id',
                ...
                'Ewave Navigation Set Links Table'
            );
    ...
    {
        $tableNavigationSetLink = $installer->getConnection()
            ->newTable($installer->getTable(self::EWAVE_NAVIGATION_MENU_SET_STORE))
            ->addColumn(
                'menu_set_id',
                \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER,
                null,
            ...
            ->addIndex(
                $installer->getIdxName(self::EWAVE_NAVIGATION_MENU_SET_STORE, ['store_id']),
                ['store_id']
            )->addForeignKey(
                $installer->getFkName(self::EWAVE_NAVIGATION_MENU_SET_STORE, 'menu_set_id',
                    self::EWAVE_NAVIGATION_MENU_SET, 'set_id'),
                'menu_set_id',
                $installer->getTable(self::EWAVE_NAVIGATION_MENU_SET),
                'set_id',
                ...
                $installer->getFkName(self::EWAVE_NAVIGATION_MENU_SET_STORE, 'store_id', 'store', 'store_id'),
                'store_id',
                $installer->getTable('store'),
                'store_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
