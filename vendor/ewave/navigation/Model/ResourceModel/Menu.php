 ...
 * Class Menu
 *
 * @package Ewave\Navigation\Model\ResourceModel
 */
class Menu extends AbstractDb
{
    ...
    public function __construct(
        ...
        \Ewave\Navigation\Helper\Data $helper,
        NotFilteredTypes $collectionFactory,
        \Ewave\Navigation\Model\ResourceModel\Type\ProcessorFactory $processorFactory,
        $connectionName = null,
        array $data = []
    ) {
    ...
    {
        $select->joinLeft(
            ['menu_set' => $this->getTable('ewave_navigation_menu_set')],
    ...
    protected function _retrieveFullMenuInfo(\Magento\Framework\DB\Select $select, $storeId)
    {
        ...
        $select->joinInner(
            ['navigation_menu_type_table' => $this->getTable('ewave_navigation_menu_item_type')],
        ...
        $select->joinLeft(
            ['menu_item_set_link' => $this->getTable('ewave_navigation_menu_set_link')],
    ...
    {
        return $this->getTable('ewave_navigation_menu_' . $typeCode . '_type');
    }
    ...
    {
        /**
         * @var $menu \Ewave\Navigation\Model\Menu
         */
        $menuId = $menu->getId();
        if ($menuId) {
    ...
    {
        return $this->data['tables_info']['ewave_navigation_menu_item_info']['fields']['to_save'] ?? [];
    }
    ...
    protected function _updateMenuItemSetLinkTable(AbstractModel $menu)
    {
        ...
        $oldIdsSelect = $this->getConnection()->select()
            ->from('ewave_navigation_menu_set_link', ['menu_set_id'])
        ...
        if (!empty($setsToInsert)) {
            ...
            $this->getConnection()->insertOnDuplicate(
                $this->getTable('ewave_navigation_menu_set_link'),
                $data
            );
        }
    ...
    public function deleteByIdAndStoreId($entityId, $storeId)
    {
        ...
        $typeCode = $connection->select()
            ->from(
                ...
                ['type_table' => $this->getTable('ewave_navigation_menu_item_type')],
    ...
    {
        return $this->getTable(self::EWAVE_NAVIGATION_MENU_ITEM_INFO_TABLE);
    }
     ...
     *
     * @param \Ewave\Navigation\Model\Menu $object
     * @param int $storeId
     * @return []
     */
    ...
    protected function saveMenuItemStoreRelation(AbstractModel $menu)
    {
        $menuId = $menu->getId();
        if ($menuId) {
            $storeRelationTable = $this->getTable('ewave_navigation_menu_item_store_relation');
