...
/**
 * Class ImageUploader
 * @package Ewave\ExtendedCustomerGroup\Model
 */
class ImageUploader
{
    /**
     ...
     */
    public function moveFileFromTmp($imageName, $imageType, $newImageName)
    {
        $baseTmpPath = $this->getBaseTmpPath();
        $basePath = $this->getBasePath();
        $baseTmpImagePath = $this->getFilePath($baseTmpPath, $imageName);
        $internalPath = $this->getInternalPath($imageType);
     ...
     * @return void
     */
    public function removeImageDirectory($imageType, $imageName)
    {
        $basePath = $this->getBasePath();
        $internalPath = $this->getInternalPath($imageType) ;
        $internalPath = $this->getFilePath($basePath, $internalPath) . $imageName;
        if ($this->mediaDirectory->isExist($internalPath)) {
            $this->mediaDirectory->delete($internalPath);
        }
    }
     ...
     * @return string
     */
    public function getImagePath($imageName, $imageType)
    {
        return $this->getFilePath(
            $this->getBasePath(),
            $this->getInternalPath($imageType) . $imageName
        );
    }
     ...
     * @return string
     */
    protected function getInternalPath($imageType)
    {
        return $imageType . DIRECTORY_SEPARATOR;
    }
