...
class PaymentMethod implements OptionSourceInterface
{
    ...
    public function toOptionArray()
    {
        if ($this->options === null) {
            $this->options = $this->getPaymentMethodList(
                self::SORTED_LIST_ITEMS,
                self::LABEL_LIST_ITEMS,
                self::WITH_GROUPS_LIST_ITEMS
            );
        }
    ...
    protected function getPaymentMethodList(
        ...
        foreach ($this->paymentHelper->getPaymentMethods() as $code => $data) {
            $title = $this->getMethodTitle($code, $data, $store);
            $methods[$code] = $title ? $title . ' [' . $code . ']' : null;
        ...
        if ($asLabelValue) {
            ...
            foreach ($methods as $code => $title) {
                if (isset($groups[$code])) {
                    $labelValues[$code]['label'] = $groups[$code];
                    if (!isset($labelValues[$code]['value'])) {
                        $labelValues[$code]['value'][$code] = ['value' => $code, 'label' => $title];
                    }
