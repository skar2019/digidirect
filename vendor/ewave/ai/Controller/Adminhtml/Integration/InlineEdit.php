    ...
    public function execute()
    {
        ...
        foreach (array_keys($postItems) as $integrationId) {
            try {
                ...
                if ($data['status'] == \Ewave\AI\Model\Integrations\Integrations::STATUS_PENDING) {
                    $integration->setStatus(\Ewave\AI\Model\Integrations\Integrations::STATUS_PENDING);
                }

                if ($data['status'] == \Ewave\AI\Model\Integrations\Integrations::STATUS_DISABLED) {
                    $integration->setStatus(\Ewave\AI\Model\Integrations\Integrations::STATUS_DISABLED);
                }
                ...
                if (isset($data['cron_time'])) {
                    if (!preg_match(\Ewave\AI\Helper\Data::CRON_REGEXP, $data['cron_time'])
